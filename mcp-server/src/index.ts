...
function formatSound(s: any): string {
  const parts: string[] = [
    `**${s.filename}**`,
    `  Path: ${s.full_path}`,
  ...
  if (s.sample_rate || s.channels || s.bit_rate || s.codec) {
    const tech: string[] = [];
    if (s.codec) tech.push(`Codec: ${s.codec}`);
    if (s.sample_rate) tech.push(`${s.sample_rate}Hz`);
    if (s.channels) tech.push(`${s.channels}ch`);
    if (s.bit_rate) tech.push(`${s.bit_rate}bps`);
    parts.push(`  ${tech.join(" | ")}`);
  }
  const meta: string[] = [];
  if (s.title) meta.push(`Title: ${s.title}`);
  if (s.artist) meta.push(`Artist: ${s.artist}`);
  if (s.album) meta.push(`Album: ${s.album}`);
  if (s.genre) meta.push(`Genre: ${s.genre}`);
  if (s.comment) meta.push(`Comment: ${s.comment}`);
...
server.tool(
  "search_sounds",
  `Search for sounds in the SFX Root database. Supports full-text search across filenames, titles, artists, albums, genres, comments, and paths. Can filter by duration range, extension, codec, channels, sample rate, and directory. Returns detailed metadata for each match.`,
  {
  ...
  async (params) => {
    ...
    try {
      ...
      if (rows.length === 0) {
        return { content: [{ type: "text", text: `No sounds found matching your criteria. Total sounds in database: ${(db.prepare("SELECT COUNT(*) as cnt FROM sound_files").get() as any)?.cnt ?? 0}` }] };
      }

      const header = `Found ${totalCount} sounds${totalCount > limit ? ` (showing ${params.offset || 0}-${(params.offset || 0) + rows.length - 1})` : ""}:\n`;
      const formatted = rows.map((r: any) => formatSound(r)).join("\n\n");
...
server.tool(
  "list_directories",
  "List all indexed directories in the SFX Root database, with file counts and last sync times.",
  {},
  async () => {
    if (!DB_PATH) {
    ...
    try {
      const rows = db.prepare("SELECT * FROM directories ORDER BY path").all() as any[];
      if (rows.length === 0) {
        db.close();
...
server.tool(
  "sound_stats",
  "Get statistics about the sound library: total files, total duration, format breakdown, codec breakdown, sample rate distribution, etc.",
  {},
  async () => {
    if (!DB_PATH) {
      return { content: [{ type: "text", text: "Error: No database path configured." }] };
    ...
    try {
      const total = (db.prepare("SELECT COUNT(*) as cnt FROM sound_files").get() as any).cnt;
      const totalSize = (db.prepare("SELECT COALESCE(SUM(file_size), 0) as s FROM sound_files").get() as any).s;
      const totalDuration = (db.prepare("SELECT COALESCE(SUM(duration_ms), 0) as d FROM sound_files").get() as any).d;
      const avgDuration = (db.prepare("SELECT COALESCE(AVG(duration_ms), 0) as d FROM sound_files WHERE duration_ms IS NOT NULL").get() as any).d;
      const dirs = (db.prepare("SELECT COUNT(*) as cnt FROM directories").get() as any).cnt;

      const byExt = db.prepare("SELECT extension, COUNT(*) as cnt FROM sound_files GROUP BY extension ORDER BY cnt DESC").all() as any[];
      const byCodec = db.prepare("SELECT codec, COUNT(*) as cnt FROM sound_files WHERE codec IS NOT NULL GROUP BY codec ORDER BY cnt DESC").all() as any[];
      const bySampleRate = db.prepare("SELECT sample_rate, COUNT(*) as cnt FROM sound_files WHERE sample_rate IS NOT NULL GROUP BY sample_rate ORDER BY cnt DESC").all() as any[];
      const byChannels = db.prepare("SELECT channels, COUNT(*) as cnt FROM sound_files WHERE channels IS NOT NULL GROUP BY channels ORDER BY cnt DESC").all() as any[];
      ...
      const lines = [
        ...
        ``,
        `### By Extension`,
        ...byExt.map((r: any) => `- ${r.extension}: ${r.cnt.toLocaleString()}`),
        ``,
        `### By Codec`,
        ...byCodec.map((r: any) => `- ${r.codec}: ${r.cnt.toLocaleString()}`),
        ``,
        `### By Sample Rate`,
        ...bySampleRate.map((r: any) => `- ${r.sample_rate}Hz: ${r.cnt.toLocaleString()}`),
        ``,
        `### By Channels`,
        ...byChannels.map((r: any) => `- ${r.channels}ch: ${r.cnt.toLocaleString()}`),
      ];
...
server.tool(
  "get_sound",
  "Get full details for a specific sound file by its database ID or file path.",
  {
  ...
  if (mode === "http" || mode === "sse") {
    ...
    if (mode === "sse") {
      ...
      httpServer.listen(port, "0.0.0.0", () => {
        ...
        if (DB_PATH) console.error(`Database: ${DB_PATH}`);
      });
    } else {
